...
public class RSHandPoseProvider : MonoBehaviour, IHandPoseProvider
{
    ...
    public HandPoseFrameData mLastFrameData = new HandPoseFrameData();
    ...
    private void UpdateFrame()
    {
        ...
        // Compute hand pose from the images
        ...
        mLastFrameData = new HandPoseFrameData();
        // fill the frame data with the results
        for (int i = 0; i < maxHands; i++)
        {
            ...
            {
                if (j < handPoseData.Keypoints.Length)
                {
                    var kpx = keypoints3d[j * 4 + 0];
                    var kpy = keypoints3d[j * 4 + 1];
                    var kpz = -keypoints3d[j * 4 + 2];
                    handPoseData.Keypoints[j] = new Vector3(kpx, kpy, kpz);
                }
            }
            mLastFrameData.Hands.Add(handPoseData);
